# K-Bestie-v3 오케스트레이션 규칙 (CLAUDE.md — v13.2)

> 최종 수정: 2026-08-12
> **(v13.2) §1-C 토큰 예산 정책 신설(`requests/004-multi.md`).** 잔여 토큰이 agy 최다 > Codex > Claude 최소로 역전되어, Claude Code를 브리프 작성·판정·게이트 확인·커밋·보고 5가지로만 한정하는 오케스트레이터 전용화 정책을 추가했다. 검증 수행 주체를 Codex 중심으로 전환하되(§1-C) 셀프통과 금지(하드룰 3)·게이트 필수성(하드룰 2)은 그대로 유지된다. 기존 위임 통로 규칙과 충돌하면 §1-C가 우선한다. 나머지 구조는 v13.1과 동일.
...
- **토큰 예산(2026-08-12 개정): 잔여량 기준 agy 최다 > Codex > Claude 최소.** 예산 배분 숫자 자체보다 Claude의 실무 직접 수행을 최소화하는 것이 핵심이다 — 상세 운영 규칙은 §1-C(토큰 예산 정책 — 오케스트레이터 전용화)를 따른다. 이 절과 §1-C가 어긋나면 §1-C가 우선한다.
...
## 1-C. 토큰 예산 정책 — Claude Code 오케스트레이터 전용화 (2026-08-12 도입, `requests/004-multi.md`)

**현재 상황**: 잔여 토큰이 agy 최다 > Codex > Claude Code 최소 순이다. Claude Code는 실무(조사·구현·리뷰·QA)를 직접 하지 않고 조율만 한다. **이 절이 §1-B·§2·§4·§5 등 기존 위임 통로 규칙과 충돌하면 이 절이 우선한다.**
...
**파일 읽기 최소화**
- 전체 파일을 읽지 않는다. 필요한 줄만 읽는다.
  - 나쁨: 파일 전체 Read. 좋음: `grep -n "패턴" 파일 | head -20` 또는 Read에 offset/limit 지정.
- 저장소 탐색이 필요하면 Claude가 직접 뒤지지 않고 Codex에 조사를 위임한다. "이 버그의 원인 파일과 함수를 찾아 20줄 이내로 보고" 형태로 던진다.
- 명령 출력은 항상 잘라 받는다. 예: `npm run typecheck 2>&1 | tail -30`, `git diff --stat`로 먼저 규모를 보고 필요한 파일만 `git diff -- <파일>`.
- 로그·JSON 전문을 화면에 쏟지 않는다. 필요한 필드만 뽑는다. 예: `jq -r '.status, .exitCode, (.touchedFiles|length)' result.json`.

**대화 길이 통제**
- 대화가 길어지면 매 턴마다 이전 내용 전체가 다시 계산된다 — 이것이 가장 큰 낭비다.
- 작업 1건이 끝나 커밋까지 하면 `/clear`로 컨텍스트를 비운다.
- 다음 작업에 필요한 인수인계는 대화에 남기지 않고 파일에 적는다(`requests/_dashboard.md` 또는 작업별 노트 파일).
- 한 대화에서 여러 작업을 이어 붙이지 않는다. 1작업 = 1대화가 기본이다.

**출력 절약**
- 응답에 코드 전문을 재출력하지 않는다. 파일 경로와 변경 요지만 쓴다.
- 대표 보고는 10줄 이내. 표나 장문 설명 금지(§15 김비서 보고 양식 자체는 그대로 유지하되 본문 분량을 이 기준으로 줄인다).
- 계획을 길게 늘어놓지 않고 바로 위임한다.

### 검증은 Codex에 맡긴다 (핵심 전환)

기존 규칙(§2, 하드룰 2·3)은 "워커 자기보고를 믿지 말고 Claude가 직접 검증"이었다. 토큰이 부족하므로 검증 **수행** 주체를 바꾼다 — 다만 셀프통과 금지(하드룰 3, 같은 워커가 자기 작업을 리뷰하지 않는다)와 게이트 자체의 필수성(하드룰 2)은 그대로 유지된다.

- agy가 구현하면 → `/codex:review --background`로 Codex가 검증한다.
- 배포 전에는 `/codex:adversarial-review --background <초점>`을 쓴다.
- Claude는 Codex의 판정 결론과 지적 항목만 읽는다. diff 전문은 읽지 않는다.
- 단, 게이트 명령(typecheck/lint/test)은 Claude가 직접 돌린다 — 이것만은 위임 불가다. 출력은 tail로 잘라 받고 실패 항목만 본다.
- agy와 Codex를 서로 교차 검증에 쓴다. 같은 워커가 자기 작업을 리뷰하지 않는다(하드룰 3과 동일 원칙).

### 위임 판단 기준

- 30초 안에 끝나는 한 줄 수정은 Claude가 직접 한다 — 브리프 작성 비용이 작업 자체보다 크면 위임이 손해다.
- 그보다 큰 것은 전부 위임한다. 애매하면 위임한다.
- 한 번에 한 워커에게 한 작업. 브리프를 뭉치지 않는다.

### 브리프 작성 원칙

브리프에 들어갈 것: 목표 / 현재 상태 / 바꿀 것 / 건드리지 말 것 / 실제 게이트 명령어 / 보고 형식 / "커밋하지 말라". 배경 설명을 길게 쓰지 않는다. 워커가 워크스페이스에서 직접 볼 수 있는 것은 브리프에 복사하지 않고 파일 경로만 알려준다.

### 금지

- 위임 가능한 작업을 Claude가 직접 하는 것
- 파일 전체 읽기, 명령 출력 전문 받기
- 커밋 후 `/clear` 없이 다음 작업을 이어가는 것
- 검증을 이유로 diff 전문을 읽는 것(Codex에 맡긴다)
