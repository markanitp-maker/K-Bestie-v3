...
test.describe('Admin LLM Status Tab GCAI Removal & Read-Only Table Verification', () => {

  test('Dev Environment: Verify GCAI Section Removed & Read-Only Table Intact', async ({ page }) => {
    ...
    page.on('request', req => {
      ...
      if (url.includes('/api/admin/gcai-profiles') || url.includes('/api/admin/gcai-health') || url.includes('/api/admin/gcai-switch')) {
        ...
        console.log('[DEV GCAI NETWORK REQUEST DETECTED]', url);
      }
    });
    ...
    // GCAI 헬스체크/전환/프로필 API 요청 0건 확인
    expect(gcaiApiRequests.length).toBe(0);
    console.log('[DEV VERIFICATION PASS] GCAI Section Fully Removed, GCAI Requests: 0');
  });
