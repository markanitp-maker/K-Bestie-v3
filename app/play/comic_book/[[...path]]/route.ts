/**
 * `/play/comic_book` reverse-proxy — K-Toon(만화책 읽기) 업스트림용 Route Handler.
 *
 ...
 *
 * MBTI 프록시와 코드가 겹치지만 이번 작업 범위는 comic_book 신설이므로
 * 기존 MBTI 파일을 건드리지 않는다. 공통화는 별도 작업으로 남긴다.
 *
 * 핸드오프 모델도 MBTI 와 같다 — K-Bestie 가 발급한 짧은 수명의 1회용
 * `play_ticket_comic_book` 쿠키(Path=/play/comic_book, HttpOnly)가 K-Toon 백엔드로
 * ticketToken 을 전달하는 유일한 통로이며, 이후 K-Toon 이 서버간 API
...
const COMIC_BOOK_PATH_PREFIX = "/play/comic_book";

const MAX_REQUEST_BODY_BYTES = 2 * 1024 * 1024;
const UPSTREAM_TIMEOUT_MS = 15_000;

/** K-Bestie 소유 쿠키만 denylist 로 막는다. K-Toon 자신의 쿠키 이름은 이 저장소가
 *  알 필요가 없다(독립 프로젝트). `play_ticket_comic_book` 은 **막으면 안 된다** —
 *  K-Toon 백엔드가 티켓 값을 받는 유일한 통로라 스트립하면 세션 확립이 깨진다. */
...
const REQUEST_HEADER_ALLOWLIST = new Set([
  ...
  "if-modified-since",
  "origin",
  // K-Toon 은 Next.js App Router 라 iframe 안 클라이언트 라우팅이 이 프록시를 다시 거친다.
  "rsc",
...
async function proxyToComicBookUpstream(request: NextRequest): Promise<Response> {
  const { pathname, search } = request.nextUrl;
  if (!pathname.startsWith(COMIC_BOOK_PATH_PREFIX)) {
    return new NextResponse("Bad Request", { status: 400 });
  }
  const suffix = pathname.slice(COMIC_BOOK_PATH_PREFIX.length);
  ...
  const upstreamOrigin = process.env.COMIC_BOOK_UPSTREAM_ORIGIN?.replace(/\/+$/, "");
  if (!upstreamOrigin) {
    console.error("[comic_book-proxy] COMIC_BOOK_UPSTREAM_ORIGIN 미설정 — /play/comic_book 프록시 불가");
    return new NextResponse("K-Toon upstream is not configured", { status: 503 });
  }
  ...
  try {
    ...
    upstreamUrl = new URL(`${COMIC_BOOK_PATH_PREFIX}${suffix}${search}`, upstreamBase);
  } catch {
    return new NextResponse("Bad Request", { status: 400 });
  }
  ...
  const isWithinUpstreamBasePath =
    ...
    (upstreamUrl.pathname === COMIC_BOOK_PATH_PREFIX ||
      upstreamUrl.pathname.startsWith(`${COMIC_BOOK_PATH_PREFIX}/`));
  ...
  try {
    ...
    if (error instanceof Error && error.name === "TimeoutError") {
      console.error("[comic_book-proxy] 업스트림 응답 시간 초과");
      return new NextResponse("Gateway Timeout", { status: 504 });
    }
    console.error("[comic_book-proxy] 업스트림 요청 실패", error);
  ...
  if (rawLocation) {
    ...
    const isSafeRedirect =
      ...
      (target.pathname === COMIC_BOOK_PATH_PREFIX ||
        target.pathname.startsWith(`${COMIC_BOOK_PATH_PREFIX}/`));
    if (!isSafeRedirect || target === null) {
      console.error(`[comic_book-proxy] 안전하지 않은 업스트림 Location 차단: ${rawLocation}`);
  ...
  const isStaticImageAsset = pathname.startsWith(`${COMIC_BOOK_PATH_PREFIX}/images/`);
  const isImmutableAsset = pathname.startsWith(`${COMIC_BOOK_PATH_PREFIX}/_next/static/`);
  ...
  for (const setCookie of upstreamResponse.headers.getSetCookie()) {
    ...
    if (!isKBestieOwnedCookie(name) && !name.startsWith("play_ticket_")) {
      ...
      console.warn(`[comic_book-proxy] 업스트림 Set-Cookie 차단(K-Bestie 소유 이름과 충돌): ${name}`);
    }
  }
...
export const GET = proxyToComicBookUpstream;
export const HEAD = proxyToComicBookUpstream;
export const POST = proxyToComicBookUpstream;
export const PUT = proxyToComicBookUpstream;
export const PATCH = proxyToComicBookUpstream;
export const DELETE = proxyToComicBookUpstream;
export const OPTIONS = proxyToComicBookUpstream;
