...
function DetailDrawer({ row, tab, onClose, onNavigate }: { row: Row; tab: Tab; onClose: () => void; onNavigate: (tab: Tab, search: string) => void }) {
  const fields = tab === "families"
    ? [
        ["가족명", row.name], ["상태", row.status], ["생성일", formatDate(row.createdAt)], ["최근 활동", formatDate(row.lastActivityAt)], ["테스트", row.testLabel], ["요금제", row.planNames?.join(" / ") || "-"],
      ]
    : tab === "parents"
      ? [
      ...
      : [
          ["아이", row.name], ["로그인 아이디", row.loginId || "-"], ["학년", row.grade], ["성별", genderLabel(row.gender)], ["가족", row.familyName], ["연결 부모", row.parents?.join(" / ") || "-"], ["승인", row.approval], ["요금제", row.planName], ["생성일", formatDate(row.createdAt)], ["최근 활동", formatDate(row.lastActivityAt)], ["최근 미션", `${row.sessionCounts?.mission ?? 0}회`], ["최근 자유대화", `${row.sessionCounts?.freechat ?? 0}회`], ["테스트", row.isTest ? "테스트" : "일반"],
        ];
...
function ExistingRequestPanel({ kind }: { kind: "restorations" | "approval" }) {
  ...
  const load = useCallback(() => { setLoading(true); fetch(endpoint).then((response) => response.ok ? response.json() : []).then((data) => setRows(Array.isArray(data) ? data : [])).finally(() => setLoading(false)); }, [endpoint]);
  useEffect(() => { load(); }, [load]);
  ...
  ]} data={visible} isLoading={loading} keyExtractor={(row) => row.id} emptyMessage="처리 대기 요청이 없습니다." />;
}
...
export default function AdminUsersPage() {
  ...
  const load = useCallback(() => { if (sub !== "all") return; setLoading(true); setError(""); fetch(`/api/admin/users/overview?${query}`).then(async (response) => { if (!response.ok) throw new Error((await response.json().catch(() => ({}))).error || "조회 실패"); return response.json(); }).then(setData).catch((reason) => setError(reason.message)).finally(() => setLoading(false)); }, [query, sub]);
  useEffect(() => { load(); }, [load]);
  ...
  const columns = tab === "families" ? [
    ...
    { key: "parents", header: "부모", render: (row: Row) => row.parents.slice(0, 2).map((parent: Row) => <div key={parent.id}>{parent.name}<span className="block max-w-48 truncate text-xs text-gray-500">{parent.email}</span></div>) },
    { key: "children", header: "아이", render: (row: Row) => row.children.map((child: Row) => child.name).join(", ") || "-" },
    { key: "plan", header: "요금제", render: (row: Row) => row.planNames.join(" / ") || "-" },
    ...
    { key: "family", header: "가족", render: (row: Row) => row.familyName },
    { key: "parents", header: "부모", render: (row: Row) => row.parents.join(" / ") || "-" },
