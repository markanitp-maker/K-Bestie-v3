    ...
    <div style={{ minHeight: "100vh", background: "var(--admin-bg)", paddingBottom: 64 }}>
      ...
      <header className="no-print" style={{ background: "var(--admin-surface)", padding: "16px 20px", borderBottom: "1px solid var(--admin-border)", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
        <div style={{ display: "flex", gap: 12, alignItems: "center" }}>
          ...
          <h1 style={{ margin: 0, fontSize: 18, fontWeight: 800, color: "var(--admin-text-primary)" }}>가족 상세 내역</h1>
        </div>
      ...
      <main id="export-area" style={{ maxWidth: 1300, margin: "0 auto", padding: "24px 20px", background: "var(--admin-bg)" }}>
        <div style={{ background: "var(--admin-surface)", padding: 24, borderRadius: 16, boxShadow: "var(--shadow-k-card)", marginBottom: 24 }}>
          <h2 style={{ fontSize: 20, margin: "0 0 16px 0", color: "var(--admin-text-primary)" }}>가족 정보 ({familyId})</h2>
          <div style={{ fontSize: 14, color: "var(--admin-text-secondary)" }}>생성일: {new Date(data.createdAt).toLocaleString()}</div>
        </div>

        <div style={{ display: "flex", gap: 24, flexWrap: "wrap" }}>
          <div style={{ flex: 1, minWidth: 300, background: "var(--admin-surface)", padding: 24, borderRadius: 16, boxShadow: "var(--shadow-k-card)" }}>
            <h3 style={{ fontSize: 16, margin: "0 0 16px 0", color: "var(--admin-text-primary)" }}>부모 목록 ({data.parents.length}명)</h3>
            {data.parents.map((p: any) => (
              <div key={p.actorId} style={{ padding: 12, border: "1px solid var(--admin-border)", borderRadius: 8, marginBottom: 8 }}>
          ...
          <div style={{ flex: 1, minWidth: 300, background: "var(--admin-surface)", padding: 24, borderRadius: 16, boxShadow: "var(--shadow-k-card)" }}>
            <h3 style={{ fontSize: 16, margin: "0 0 16px 0", color: "var(--admin-text-primary)" }}>아이 목록 ({data.children.length}명)</h3>
            {data.children.map((c: any) => (
              <div key={c.childId} style={{ padding: 12, border: "1px solid var(--admin-border)", borderRadius: 8, marginBottom: 8 }}>
        ...
        <div style={{ background: "var(--admin-surface)", padding: 24, borderRadius: 16, boxShadow: "var(--shadow-k-card)", marginTop: 24 }}>
          <h3 style={{ fontSize: 16, margin: "0 0 16px 0", color: "var(--admin-text-primary)" }}>커넥션 퍼널 (최근 30일)</h3>
          <AdminDataTable
            columns={[
              { key: "id", header: "아이 ID", render: (r: any) => `${r.childId.split("-")[0]}...` },
              { key: "mission", header: "마지막 미션완료", render: (r: any) => r.missionCompletedAt ? "✅" : "-" },
              { key: "report_gen", header: "리포트 생성", render: (r: any) => r.reportGeneratedAt ? "✅" : "-" },
              { key: "report_view", header: "부모가 조회", render: (r: any) => r.reportViewedAt ? "✅" : "-" },
              { key: "topic_view", header: "토픽 확인", render: (r: any) => r.topicViewedAfter ? "✅" : "-" },
            ]}
            data={data.connectionFunnel}
            keyExtractor={(r: any) => r.childId}
          />
        </div>
      </main>
    </div>
