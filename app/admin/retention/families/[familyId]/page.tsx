    ...
    <div style={{ minHeight: "100vh", background: "var(--hb-bg, #fafaf8)", paddingBottom: 64 }}>
      ...
      <header className="no-print" style={{ background: "var(--hb-card)", padding: "16px 20px", borderBottom: "1px solid var(--hb-border)", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
        <div style={{ display: "flex", gap: 12, alignItems: "center" }}>
          ...
          <h1 style={{ margin: 0, fontSize: 18, fontWeight: 800, color: "#1e1e2d" }}>가족 상세 내역</h1>
        </div>
      ...
      <main id="export-area" style={{ maxWidth: 1000, margin: "0 auto", padding: "24px 20px", background: "var(--hb-bg, #fafaf8)" }}>
        <div style={{ background: "var(--hb-card)", padding: 24, borderRadius: 16, boxShadow: "var(--hb-shadow)", marginBottom: 24 }}>
          <h2 style={{ fontSize: 20, margin: "0 0 16px 0", color: "#1e1e2d" }}>가족 정보 ({familyId})</h2>
          <div style={{ fontSize: 14, color: "var(--hb-muted)" }}>생성일: {new Date(data.createdAt).toLocaleString()}</div>
        </div>

        <div style={{ display: "flex", gap: 24, flexWrap: "wrap" }}>
          <div style={{ flex: 1, minWidth: 300, background: "var(--hb-card)", padding: 24, borderRadius: 16, boxShadow: "var(--hb-shadow)" }}>
            <h3 style={{ fontSize: 16, margin: "0 0 16px 0", color: "#1e1e2d" }}>부모 목록 ({data.parents.length}명)</h3>
            {data.parents.map((p: any) => (
              <div key={p.actorId} style={{ padding: 12, border: "1px solid var(--hb-border)", borderRadius: 8, marginBottom: 8 }}>
          ...
          <div style={{ flex: 1, minWidth: 300, background: "var(--hb-card)", padding: 24, borderRadius: 16, boxShadow: "var(--hb-shadow)" }}>
            <h3 style={{ fontSize: 16, margin: "0 0 16px 0", color: "#1e1e2d" }}>아이 목록 ({data.children.length}명)</h3>
            {data.children.map((c: any) => (
              <div key={c.childId} style={{ padding: 12, border: "1px solid var(--hb-border)", borderRadius: 8, marginBottom: 8 }}>
        ...
        <div style={{ background: "var(--hb-card)", padding: 24, borderRadius: 16, boxShadow: "var(--hb-shadow)", marginTop: 24 }}>
          <h3 style={{ fontSize: 16, margin: "0 0 16px 0", color: "#1e1e2d" }}>커넥션 퍼널 (최근 30일)</h3>
          <table style={{ width: "100%", borderCollapse: "collapse", textAlign: "left", fontSize: 14 }}>
            <thead>
              <tr style={{ background: "var(--hb-primary-light)", color: "var(--hb-muted)" }}>
                <th style={{ padding: "8px 12px", borderBottom: "1px solid var(--hb-border)" }}>아이 ID</th>
                <th style={{ padding: "8px 12px", borderBottom: "1px solid var(--hb-border)" }}>마지막 미션완료</th>
                <th style={{ padding: "8px 12px", borderBottom: "1px solid var(--hb-border)" }}>리포트 생성</th>
                <th style={{ padding: "8px 12px", borderBottom: "1px solid var(--hb-border)" }}>부모가 조회</th>
                <th style={{ padding: "8px 12px", borderBottom: "1px solid var(--hb-border)" }}>토픽 확인</th>
              </tr>
            </thead>
            <tbody>
              {data.connectionFunnel.map((f: any) => (
                <tr key={f.childId} style={{ borderBottom: "1px solid var(--hb-border)" }}>
                  <td style={{ padding: "8px 12px", color: "#1e1e2d" }}>{f.childId.split("-")[0]}...</td>
                  <td style={{ padding: "8px 12px", color: "#1e1e2d" }}>{f.missionCompletedAt ? "✅" : "-"}</td>
                  <td style={{ padding: "8px 12px", color: "#1e1e2d" }}>{f.reportGeneratedAt ? "✅" : "-"}</td>
                  <td style={{ padding: "8px 12px", color: "#1e1e2d" }}>{f.reportViewedAt ? "✅" : "-"}</td>
                  <td style={{ padding: "8px 12px", color: "#1e1e2d" }}>{f.topicViewedAfter ? "✅" : "-"}</td>
                </tr>
              ))}
            </tbody>
          </table>
