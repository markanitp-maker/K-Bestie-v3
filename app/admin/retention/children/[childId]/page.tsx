    ...
    <div style={{ minHeight: "100vh", background: "var(--hb-bg, #fafaf8)", paddingBottom: 64 }}>
      ...
      <header className="no-print" style={{ background: "var(--hb-card)", padding: "16px 20px", borderBottom: "1px solid var(--hb-border)", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
        <div style={{ display: "flex", gap: 12, alignItems: "center" }}>
          ...
          <h1 style={{ margin: 0, fontSize: 18, fontWeight: 800, color: "#1e1e2d" }}>아이 상세 내역</h1>
        </div>
      ...
      <main id="export-area" style={{ maxWidth: 1000, margin: "0 auto", padding: "24px 20px", background: "var(--hb-bg, #fafaf8)" }}>
        <div style={{ background: "var(--hb-card)", padding: 24, borderRadius: 16, boxShadow: "var(--hb-shadow)", marginBottom: 24 }}>
          <h2 style={{ fontSize: 20, margin: "0 0 16px 0", color: "#1e1e2d" }}>아이 정보 ({childId})</h2>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(250px, 1fr))", gap: 16, marginBottom: 24 }}>
          <div style={{ background: "var(--hb-card)", padding: 16, borderRadius: 12, boxShadow: "var(--hb-shadow)" }}>
            <h3 style={{ fontSize: 15, margin: "0 0 12px 0", color: "#1e1e2d" }}>미션 활동</h3>
            <div style={{ fontSize: 14, color: "var(--hb-muted)" }}>시작: {data.mission.startCount}회 | 완료: {data.mission.completeCount}회</div>
            <div style={{ fontSize: 14, color: "var(--hb-muted)", marginTop: 4 }}>완료율: {data.mission.completionRate ? (data.mission.completionRate * 100).toFixed(1) + "%" : "-"}</div>
          </div>
          <div style={{ background: "var(--hb-card)", padding: 16, borderRadius: 12, boxShadow: "var(--hb-shadow)" }}>
            <h3 style={{ fontSize: 15, margin: "0 0 12px 0", color: "#1e1e2d" }}>자유 대화</h3>
            <div style={{ fontSize: 14, color: "var(--hb-muted)" }}>시작: {data.freechat.startCount}회 | 완료: {data.freechat.completeCount}회</div>
          </div>
          <div style={{ background: "var(--hb-card)", padding: 16, borderRadius: 12, boxShadow: "var(--hb-shadow)" }}>
            <h3 style={{ fontSize: 15, margin: "0 0 12px 0", color: "#1e1e2d" }}>놀이 활동</h3>
            <div style={{ fontSize: 14, color: "var(--hb-muted)" }}>시작: {data.play.startCount}회 | 완료: {data.play.completeCount}회</div>
            <div style={{ fontSize: 13, color: "var(--hb-muted)", marginTop: 8 }}>
              {Object.entries(data.play.byType || {}).map(([k, v]) => (
                <span key={k} style={{ marginRight: 8, display: "inline-block", background: "var(--hb-bg)", padding: "2px 6px", borderRadius: 4 }}>{k}: {v as number}회</span>
              ))}
            </div>
          </div>
        </div>
            ...
            <table style={{ width: "100%", borderCollapse: "collapse", textAlign: "left" }}>
              <thead style={{ position: "sticky", top: 0, background: "var(--hb-primary-light)" }}>
                <tr>
                  ...
                  <th style={{ padding: "8px 12px", borderBottom: "1px solid var(--hb-border)", color: "var(--hb-muted)" }}>기능/모드</th>
                </tr>
              </thead>
              <tbody>
                {data.timeline.map((e: any, i: number) => (
                  <tr key={i} style={{ borderBottom: "1px solid var(--hb-border)" }}>
                    ...
                    <td style={{ padding: "8px 12px", color: "var(--hb-muted)" }}>{e.feature} {e.playType ? `(${e.playType})` : ""} {e.conversationMode ? `[${e.conversationMode}]` : ""}</td>
                  </tr>
                ))}
              </tbody>
            </table>
