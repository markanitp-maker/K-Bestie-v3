...
export async function GET(req: NextRequest) {
  ...
  const limit = resolveAnalyticsResultLimit(req.nextUrl.searchParams);
  try {
    ...
    const childIdsByFamily = new Map<string, Set<string>>();
    for (const [childId, familyId] of identity.childFamily) {
      const childIds = childIdsByFamily.get(familyId) ?? new Set<string>();
      childIds.add(childId);
      childIdsByFamily.set(familyId, childIds);
    }
    const parentIdsByFamily = new Map<string, Set<string>>();
    for (const [parentId, familyId] of identity.parentFamily) {
      const parentIds = parentIdsByFamily.get(familyId) ?? new Set<string>();
      parentIds.add(parentId);
      parentIdsByFamily.set(familyId, parentIds);
    }
    const eventsByFamily = groupAnalyticsRowsByFamily(events, (row) => row.family_id
      ?? (row.child_id ? identity.childFamily.get(row.child_id) : undefined)
      ?? (row.actor_id ? identity.parentFamily.get(row.actor_id) : undefined));
    const sessionsByFamily = groupAnalyticsRowsByFamily(sessions, (row) => identity.childFamily.get(row.child_id));
    const questionsByFamily = groupAnalyticsRowsByFamily(questions, (row) => identity.childFamily.get(row.child_id));
    const viewsByFamily = groupAnalyticsRowsByFamily(views, (row) => {
      const childId = reportChild.get(row.report_id);
      return childId ? identity.childFamily.get(childId) : undefined;
    });
    const rows = [...identity.familyIds].map((familyId) => {
      const childIds = childIdsByFamily.get(familyId) ?? new Set<string>();
      const parentIds = parentIdsByFamily.get(familyId) ?? new Set<string>();
      const familyEvents = eventsByFamily.get(familyId) ?? [];
      const familySessions = sessionsByFamily.get(familyId) ?? [];
      const familyQuestions = questionsByFamily.get(familyId) ?? [];
      const familyViews = viewsByFamily.get(familyId) ?? [];
    ...
    return NextResponse.json({
      filters,
      families: limitedRows,
      meta: {
        identifiers: "opaque_refs",
        total: rows.length,
        limit,
