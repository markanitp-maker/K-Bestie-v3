...
export async function GET(req: Request) {
  ...
  const results = { sent: 0, inboxOnly: 0, skippedNoReport: 0, skippedAlreadySent: 0, failed: 0 };

  const { data: parents, error: parentsError } = await db.from("parents").select("id,report_push_enabled").eq("account_status", "ACTIVE");
  ...
  for (const parent of parents ?? []) {
    ...
    const pushEnabled = parent.report_push_enabled !== false && preference?.parent_report_enabled !== false;
    ...
    const inboxNotification = await createInboxNotification(db, {
      userId: parent.id,
      childId: reports.length === 1 ? reports[0].child_id : null,
      role: "parent",
      type: "report",
      title,
      body: "어제의 이야기와 오늘 나눌 대화를 확인해 보세요.",
      targetUrl: url,
      sourceId: reports.map((report) => report.id).sort().join(","),
      idempotencyKey: `report:${parent.id}:${yesterday}:daily`,
    });
    if (!pushEnabled) { results.inboxOnly++; continue; }
    ...
    for (const subscription of subscriptions ?? []) {
      try {
        await sendPushNotificationWithRetry({ endpoint: subscription.endpoint, keys: { p256dh: subscription.p256dh, auth: subscription.auth } }, { title, body: "어제의 이야기와 오늘 나눌 대화를 확인해 보세요.", url, notificationId: inboxNotification.id });
        successful++;
      } catch (error) {
