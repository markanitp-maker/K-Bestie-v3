...
export default function ChatPage() {
  ...
  const handleTurnComplete = useCallback((turn: Turn) => {
    ...
    if (sid) {
      ...
      const releaseWriteToken = acquireHazardToken("freechat", "pending_message_write");
  ...
  // Publish Free Chat activity state SSOT
  useEffect(() => {
    const isChatActive = status === "connecting" || status === "live";
    publishConversationActivity("freechat", isChatActive);
    return () => {
      publishConversationActivity("freechat", false);
    };
  }, [status]);

  // Specific FreeChat Hazard Tokens
  useEffect(() => {
    if (isRecording) {
      const release = acquireHazardToken("freechat", "recording");
      return () => release();
    }
  }, [isRecording]);
  ...
  useEffect(() => {
    if (isResponding) {
      const release = acquireHazardToken("freechat", "responding");
      return () => release();
    }
  }, [isResponding]);
  ...
  useEffect(() => {
    if (isSpeaking) {
      const release = acquireHazardToken("freechat", "speaking");
      return () => release();
    }
  }, [isSpeaking]);
  ...
  useEffect(() => {
    if (dailyReward || exitAfterRewardRef.current) {
      const release = acquireHazardToken("freechat", "reward_modal");
      return () => release();
    }
  }, [dailyReward]);
  ...
  useEffect(() => {
    ...
    const finalizeDailyReward = async () => {
      const releaseRewardToken = acquireHazardToken("freechat", "pause_reward_settlement");
      try {
  ...
  const restoreSession = useCallback(async (cId: string) => {
    const releaseRestoreToken = acquireHazardToken("freechat", "start_restore");
  ...
  const handleStart = useCallback(async () => {
    ...
    if (isActivationBarrierActive()) {
      console.warn("[freechat] Start blocked by activation barrier");
      return;
    }
    const releaseStartToken = acquireHazardToken("freechat", "start_restore");
    try {
      setReportDone(false);
      setReportError(null);
