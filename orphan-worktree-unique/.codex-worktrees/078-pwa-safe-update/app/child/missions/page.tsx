...
function MissionInner({ onTextModeChange }: { onTextModeChange?: (isTextMode: boolean) => void }) {
  ...
  // Publish Mission activity state SSOT & Readiness
  useEffect(() => {
  ...
  useEffect(() => {
    const isMissionActive =
      ...
      entryStatus === "resuming" ||
      entryStatus === "active";
    publishConversationActivity("mission", isMissionActive);
    return () => {
  ...
  useEffect(() => {
    if (
      ...
      const release = tryAcquireConversationHazard("mission", "checking_starting_resuming");
      return () => release?.();
    }
  }, [entryStatus, phase]);

  useEffect(() => {
    if (entryStatus === "active" || missionState === "active") {
      const release = tryAcquireConversationHazard("mission", "active_session");
      return () => release?.();
    }
  }, [entryStatus, missionState]);

  useEffect(() => {
    if (isProcessingAnswer || turnPhaseUi !== "idle") {
      const release = tryAcquireConversationHazard("mission", "turn_processing");
      return () => release?.();
    }
  }, [isProcessingAnswer, turnPhaseUi]);

  useEffect(() => {
    if (missionState === "completing" || isRewardModalOpen) {
      const release = tryAcquireConversationHazard("mission", "completing_reward");
      return () => release?.();
    }
  }, [missionState, isRewardModalOpen]);
  ...
  const runMissionRequest = useCallback(async (
    ...
    if (isActivationBarrierActive()) {
      console.warn("[mission] Mission request blocked by activation barrier");
      setErrorMsg("앱 업데이트가 준비 중입니다. 잠시 후 다시 시도해 주세요.");
      setPhase("error");
      setEntryStatus("error");
      return;
    }
  ...
  useEffect(() => {
    ...
    void runMissionRequest(async (request) => {
      ...
      if (!request.isActive()) return;
      if (versionGate.status === "reload_started" && versionGate.serverBuildId !== "local") {
        request.markSettled();
        return;
      }
      if (versionGate.status !== "ready" && versionGate.serverBuildId !== "local") {
