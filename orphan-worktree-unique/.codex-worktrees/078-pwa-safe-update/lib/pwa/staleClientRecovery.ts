...
export const ALLOWLISTED_STALE_ERROR_CODES = [
  "EXHAUSTED",
  "TOO_SOON",
  "OFFLINE",
  "UNSUPPORTED",
...
export type StaleRecoveryErrorCode = (typeof ALLOWLISTED_STALE_ERROR_CODES)[number];

export function mapStaleRecoveryErrorCode(error: unknown): StaleRecoveryErrorCode {
  if (typeof error === "string") {
    const upper = error.toUpperCase().trim();
    if (upper === "EXHAUSTED") return "EXHAUSTED";
    if (upper === "TOO_SOON") return "TOO_SOON";
    if (upper === "OFFLINE") return "OFFLINE";
    if (upper === "UNSUPPORTED") return "UNSUPPORTED";
    if (upper === "CONTROLLER_MISMATCH") return "CONTROLLER_MISMATCH";
    if (upper === "INVALID_ENVELOPE") return "INVALID_ENVELOPE";
    if (upper === "SERVER_CHECK_FAILED") return "SERVER_CHECK_FAILED";
    if (upper === "TRIPLE_MISMATCH") return "TRIPLE_MISMATCH";
  }
  return "EXCEPTION";
}
