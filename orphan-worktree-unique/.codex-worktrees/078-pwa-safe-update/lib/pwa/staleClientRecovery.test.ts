...
test("stale recovery pending marker lifecycle: reload success clears marker", () => {
  const session = memorySessionStorage();
  const PENDING_KEY = "pwa_stale_recovery_pending";

  session.setItem(PENDING_KEY, "true");
  assert.equal(session.getItem(PENDING_KEY), "true");

  // On mount check
  const pending = session.getItem(PENDING_KEY);
  if (pending) {
    session.setItem(PENDING_KEY, ""); // Cleared
  }

  assert.equal(session.getItem(PENDING_KEY), "");
});

test("mapStaleRecoveryErrorCode maps errors to allowlisted fixed string codes", () => {
  const { mapStaleRecoveryErrorCode } = require("./staleClientRecovery");
  assert.equal(mapStaleRecoveryErrorCode("EXHAUSTED"), "EXHAUSTED");
  assert.equal(mapStaleRecoveryErrorCode("too_soon"), "TOO_SOON");
  assert.equal(mapStaleRecoveryErrorCode("offline"), "OFFLINE");
  assert.equal(mapStaleRecoveryErrorCode("unsupported"), "UNSUPPORTED");
  assert.equal(mapStaleRecoveryErrorCode("controller_mismatch"), "CONTROLLER_MISMATCH");
  assert.equal(mapStaleRecoveryErrorCode("invalid_envelope"), "INVALID_ENVELOPE");
  assert.equal(mapStaleRecoveryErrorCode("unknown raw error message: 500 internal"), "EXCEPTION");
  assert.equal(mapStaleRecoveryErrorCode(new Error("raw exception message")), "EXCEPTION");
});
