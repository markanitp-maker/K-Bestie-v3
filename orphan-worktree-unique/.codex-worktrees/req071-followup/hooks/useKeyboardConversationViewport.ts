...
export function useKeyboardConversationViewport() {
  ...
  const [viewportPageTop, setViewportPageTop] = useState(0);
  ...
  useEffect(() => {
    ...
    const updateViewport = () => {
      ...
      const currentPageTop = visualViewport
        ? visualViewport.pageTop
        : window.scrollY;
      setViewportHeight(currentHeight);
      setViewportPageTop(currentPageTop);
  ...
  return { viewportHeight, viewportPageTop, isKeyboardOpen };
}
