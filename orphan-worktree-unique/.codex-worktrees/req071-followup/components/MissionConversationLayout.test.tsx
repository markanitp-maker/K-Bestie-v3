...
Object.defineProperty(visualViewport, "pageTop", { configurable: true, value: 300 });
...
test("녹음 상태가 남아 있어도 텍스트 모드에서는 K 상태 뱃지를 대기 중으로 렌더한다", async () => {
  ...
  const viewportMetrics: Array<{ height: number; pageTop: number } | null> = [];

  await act(async () => {
    root.render(
      <MissionConversationLayout
        onClose={() => {}}
        onKeyboardViewportMetricsChange={(metrics) => viewportMetrics.push(metrics)}
        progressCurrent={0}
        progressTotal={5}
        history={[]}
  ...
  assert.deepEqual(viewportMetrics.at(-1), { height: 500, pageTop: 300 });
