      ...
      <div className="w-full max-w-[480px] min-w-0 h-[100dvh] relative box-border grid grid-cols-1 grid-rows-[auto_minmax(0,1fr)_auto_auto]" style={{ background: "linear-gradient(180deg, #D8EEFF 0%, #EAF6FB 46%, #FFF9EE 76%, #FFF4E6 100%)" }}>
        ...
        <div className="relative z-10 flex flex-col shrink-0 w-full min-w-0 max-w-full">
          ...
          {progressDisplay === "ambient" ? (
            <div
              className="mt-[calc(61px+env(safe-area-inset-top))] mx-auto flex h-[clamp(46px,6dvh,50px)] w-[90%] max-w-[400px] shrink-0 items-center rounded-full bg-white/90 px-[clamp(16px,4vw,20px)] shadow-[0_3px_10px_rgba(75,85,99,0.10)]"
              aria-label="오늘 이야기를 즐겁게 나누는 중"
            >
              <div className="relative h-3 w-full overflow-hidden rounded-full bg-[#E7EEF4]">
                <div
                  className="h-full rounded-full bg-gradient-to-r from-[#FFD36A] via-[#FF9B4A] to-[#F47B42] transition-[width] duration-700 ease-out"
                  style={{ width: `${Math.max(18, Math.min(100, (progressCurrent / Math.max(progressTotal, 1)) * 100))}%` }}
                />
                <div className="absolute inset-y-0 left-0 w-1/3 animate-pulse rounded-full bg-white/35 motion-reduce:animate-none" />
              </div>
            </div>
          ) : (
