...
import { MissionV3Conversation } from "./MissionV3Conversation";
...
function MissionRouteGate() {
  ...
  const requestedChildId = searchParams.get("childId")?.trim() ?? "";
  ...
  const [normalMissionPolicy, setNormalMissionPolicy] = useState<"loading" | "legacy" | "v3">("loading");
  const [normalChildId, setNormalChildId] = useState<string | null>(null);
  useEffect(() => {
    let cancelled = false;
  ...
  useEffect(() => {
    ...
    const resolvePolicy = async () => {
      setNormalMissionPolicy("loading");
      try {
        let childId = requestedChildId;
        if (!childId) {
          const childResponse = await fetch("/api/child/me");
          if (!childResponse.ok) throw new Error("아이 프로필을 확인하지 못했어요.");
          const childData: unknown = await childResponse.json();
          if (!childData || typeof childData !== "object" || !("id" in childData) || typeof childData.id !== "string") {
            throw new Error("아이 프로필 응답이 올바르지 않아요.");
          }
          childId = childData.id;
        }

        const policyResponse = await fetch(
          `/api/mission/v3/today-progress?childId=${encodeURIComponent(childId)}`,
        );
        if (!policyResponse.ok) throw new Error("미션 정책을 확인하지 못했어요.");
        const policyData: unknown = await policyResponse.json();
        const isV3 = Boolean(
          policyData
          && typeof policyData === "object"
          && "policyVersion" in policyData
          && policyData.policyVersion === "v3_single_daily",
        );
        if (!cancelled) {
          setNormalChildId(childId);
          setNormalMissionPolicy(isV3 ? "v3" : "legacy");
        }
      } catch (error) {
        console.error("[MissionRouteGate] v3 정책 조회 실패, 레거시 미션으로 전환합니다.", error);
        if (!cancelled) {
          setNormalChildId(null);
          setNormalMissionPolicy("legacy");
        }
      }
    };
    void resolvePolicy();
    return () => { cancelled = true; };
  }, [decision, requestedChildId]);
  ...
  if (normalMissionPolicy === "loading") {
    return (
      <div className="flex items-center justify-center" style={{ height: "100dvh", background: "var(--color-k-surface)" }}>
        <div className="w-8 h-8 rounded-full border-2 border-t-transparent animate-spin" style={{ borderColor: "var(--color-k-orange) var(--color-k-orange) transparent transparent" }} />
      </div>
  ...
  return (
    <DemoFrame>
      <div className="mission-frame-wrapper relative w-full h-full">
        ...
        `}} />
        {normalMissionPolicy === "v3" && normalChildId ? (
          <MissionV3Conversation childId={normalChildId} onTextModeChange={setIsTextMode} />
        ) : (
