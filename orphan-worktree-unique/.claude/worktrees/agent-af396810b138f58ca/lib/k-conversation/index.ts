...
import { handleChosungTurn } from "./chosungGame/gameEngine";
...
export async function respond(
  ...
  const chosungResult = input.mode === "FREE_CHAT" && gradePersona
    ? await handleChosungTurn(deps.db, {
      childId: input.childId,
      sessionId: input.sessionId,
      currentUtterance: input.currentUtterance,
    }, {
      ...
      signals,
      boredom,
      generate: (args) => generateResponse({
        ai: deps.ai,
        ...
        input: {
          ...
          recentHistory,
          adapterInstruction: args.instruction,
          isGeneralKnowledgeQuestion: false,
        },
      }),
    })
    : null;
  if (chosungResult) {
    return {
      text: chosungResult.text,
      action: chosungResult.action,
      category: "generated",
      boredom,
      memoryTiersUsed: memorySnapshot.tiersUsed,
      tokenIn: chosungResult.tokenIn,
      tokenOut: chosungResult.tokenOut,
    };
  }
