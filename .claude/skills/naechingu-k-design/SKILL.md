...
description: "내친구 케이 브랜드 디자인 시스템. 앱 화면, 대시보드, 웹페이지, 랜딩페이지, UI 컴포넌트, 슬라이드 등 무엇이든 '내친구 케이' 브랜드 톤으로 만들 때 사용한다. 색상(딥그린/라이트그린/코랄), 폰트(Pretendard/Gaegu), 둥근 모서리, 넉넉한 여백, 따뜻하고 신뢰감 있는 톤을 일관되게 적용한다. React+Vite+TypeScript+Tailwind v3 스택 기준. 'brand', '디자인', '색상', '톤', '내친구 케이' 스타일로 만들라는 요청 시 트리거."
