...
const TEST_CHILD_ID = '4e7c1a6f-a953-4ebc-a181-e9c054a8ee3c'; // TestA (is_test_account=true)
...
test('060: manual reporting run shows child name/login instead of raw UUID', async ({ page, context }) => {
  ...
  await searchBox.fill('TestA');
  await page.waitForTimeout(1000);

  await page.locator('tr', { hasText: 'TestA' }).first().click();
  ...
  console.log('[060-dev] TestA name shown in results:', bodyText.includes('TestA'));
