...
test('PROD MBTI verify fix', async ({ page }) => {
  ...
  await page.screenshot({ path: '/tmp/mbti-prod-verify.png', fullPage: true });
  ...
  console.log('[VERIFY] network errors:', networkErrors.length);
  ...
  const hasLoadError = bodyText.includes('불러오다가 문제가 생겼어요');

  expect(hasCspBlock, 'CSP frame-ancestors block should NOT occur').toBe(false);
  expect(hasLoadError, 'MBTI should not show its own load-error screen').toBe(false);
