# K-Bestie-v3 Claude 보조 규칙 (CLAUDE.md — v18)
...
**활용 비율 (2026-08-21 대표 지시).** 이 비율이 위임 판단의 기준이다.

| 주체 | 비중 | 담당 |
|---|---|---|
| **Claude Code** | 40% | 오케스트레이션(분해·디스패치·게이트 판정·통합·커밋·Dev 배포·보고) **＋ 설계, 어려운 코딩, 어려운 QA** |
| **agy** | 50% | **10분 이내로 끝나는 것 전부.** 단순 코딩, 단순 QA, 리서치, 전수 조사, 점검·확인, 잡무 |
| **Codex** | 10% | **코드 리뷰, 대량 작업.** |

판단 순서:
1. 10분 이내 단순 작업인가 → **agy**
2. 코드 리뷰인가, 아니면 한 번에 훑어야 할 대량 작업인가 → **Codex**
3. 설계·어려운 코딩·어려운 QA인가 → **내가 직접 한다**
...
- **코드 리뷰는 Codex가 기본이다.** 셀프 통과 금지(§0-3)는 그대로다 — 내가 짠 것을 내가
  통과시키지 않는다. Codex 쿼터가 소진되면 agy 리뷰로 임시 대체한다.
- **어려운 일이 내 몫으로 돌아왔다.** 예전처럼 "직접 구현 금지" 가 아니다.
  다만 쉬운 일을 내가 하는 것은 여전히 낭비다 — 10분짜리는 agy 로 보낸다.
