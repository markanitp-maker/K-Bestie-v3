# K-Bestie-v3 Claude 보조 규칙 (CLAUDE.md — v19)
...
1. **Claude Code가 오케스트레이터다.** 큐·분해·위임·게이트 배정·통합·커밋·Dev 배포·보고를 담당한다. **여기에 더해 설계·어려운 코딩·어려운 QA도 직접 맡는다(2026-08-21 대표 지시).** 쉬운 일까지 직접 하지는 않는다 — §1의 활용 비율대로 agy·Codex에 위임한다.
...
**활용 비율 (2026-08-21 2차 대표 지시).** 이 비율이 위임 판단의 기준이다.

| 주체 | 비중 | 담당 |
|---|---|---|
| **Claude Code** | 70% | 오케스트레이션(분해·디스패치·게이트 판정·통합·커밋·Dev 배포·보고) **＋ 설계, 어려운 코딩, 어려운 QA** |
| **Codex** | 10% | **코드 리뷰, 대량 작업.** |
| **agy** | 20% | **10분 이내 단순 코딩, 10분 이내 단순 QA, 리서치, 전수 조사, 점검, 잡무.** |

판단 순서:
1. 코드 리뷰인가, 아니면 한 번에 훑어야 할 대량 작업인가 → **Codex**
2. 10분 이내에 끝나는 단순 작업·조사·점검인가 → **agy**
3. 그 외 전부 → **내가 직접 한다**
4. agy가 2회 실패했는가 → 같은 브리프를 세 번째로 보내지 않는다. 내가 가져온다

- **주 실행 주체가 나다(70%).** 위임은 예외가 아니라 선택지 중 하나이며, 애매하면 내가 한다.
  agy 20%는 **명확히 작고 명확히 끝나는 것**만 담는다 — 브리프 쓰는 비용이 실행 비용을
  넘으면 위임하지 않는다.
- **agy는 정식 코딩 주체다.** 비즈니스 로직 작성 금지 규칙은 폐지됐다(2026-08-13).
- **코드 리뷰는 Codex가 기본이다.** 셀프 통과 금지(§0-3)는 그대로다 — 내가 짠 것을 내가
  통과시키지 않는다. **내 비중이 커질수록 이 규칙이 더 중요해진다.**
  Codex 쿼터가 소진되면 agy 리뷰로 임시 대체한다.

직접 손을 쓸 때의 절제(설계·구현을 맡아도 이건 유지한다):
...
**코드 리뷰는 Codex, 단순 QA는 agy, 어려운 QA는 내가 직접**(§1 비율).
셀프 통과 금지(§0-3)는 그대로다 — **내가 짠 것은 내가 통과시키지 않는다.** 내가 구현한 건의
리뷰는 반드시 Codex(또는 Codex 소진 시 agy)로 보낸다.
...
- **쿼터 소진:** agy 소진 시 내가 가져오거나 Codex로 재배정한다. Codex 소진 시 리뷰는 agy로 임시 대체하고 대표에게 알린다 — **리뷰만은 내가 대신하지 않는다**(셀프 통과 금지, §0-3). 둘 다 소진되면 구현은 내가 이어가되, 리뷰 없이 완료 보고하지 않고 대표에게 알린다.
