...
- **12-G QA 실행 주체 에스컬레이션(2026-08-10 대표님 확정)**: agy가 같은 QA 대상에서 **2회 연속** 결과 없이 실패(타임아웃 등 인프라 실패)하면 agy 재시도를 멈추고 Codex(read-only 아닌 실행 가능 세션)에게 동일 시나리오의 Playwright 작성·실행을 **1회** 위임한다. Codex도 실행하지 못하거나(§1-B상 역할 경계로 자체 거부하는 경우 포함) 실패하면, 메인 Claude가 직접 Playwright를 작성·실행해 게이트②를 완료한다. 이 경로로 Claude가 QA를 수행한 경우도 셀프통과가 아니다 — 실제 브라우저 실행 증거(스크린샷/콘솔로그)를 남기고 결과를 있는 그대로 보고한다.
