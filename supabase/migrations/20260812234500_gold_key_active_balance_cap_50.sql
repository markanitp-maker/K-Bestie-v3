-- ⚠️ 재적용 금지 (2026-08-14)
-- 이 파일은 award_mission_v3_reward 를 "목표 정확히 4개 / 임계 3" 기준으로 정의한다.
-- Mission v3 완료 기준이 "목표 10개 생성 / 5개 달성"으로 바뀐 뒤라, 이 파일을
-- 단독 재적용하면 아이가 미션을 완료해도 goals_not_initialized 로 거부되어
-- 황금열쇠가 지급되지 않고 COMPLETED 전이도 일어나지 않는다.
-- (2026-08-14 Production 실측 — 실제로 이 사고가 났다)
--
-- 최신 정의는 20260814100500_award_mission_v3_reward_threshold_10_5.sql 이다.
-- 마이그레이션은 반드시 타임스탬프 순으로 전체 적용하고, 이 파일만 골라
-- 재실행하지 마라.

-- 대표님 승인(2026-08-12): 출석 룰렛이 1회 스핀으로 최대 +9까지 지급할 수 있어
-- 기존 활성 골드키 보유 상한 22개가 너무 좁다. 22 -> 50으로 상향한다.
-- 이 마이그레이션이 손대는 함수는 089(하루 1회 미션보상 통합)가 만든
-- 20260812220000_mission_daily_reward_all_writers.sql 의 4개 writer와,
-- 089와 무관한 별도 writer인 complete_freechat_daily_engagement 다.
-- 상한 값 외 로직은 전혀 바꾸지 않는다.
-- Note: no explicit BEGIN/COMMIT — scripts/apply-migration.js provides its own
-- implicit-transaction atomicity via a single simple-query HTTP call.
...
BEGIN
  ...
  IF v_completed THEN
    ...
    ELSIF v_active_balance >= 50 THEN
...
BEGIN
  ...
  IF v_newly_completed THEN
    ...
    -- Preserve the existing child-scoped wallet serialization and 50-key cap.
    PERFORM pg_advisory_xact_lock(hashtext(p_child_id::text));
...
CREATE OR REPLACE FUNCTION public.complete_freechat_daily_engagement(p_child_id uuid, p_environment text, p_source_session_id uuid, p_turn_count integer, p_completed_at timestamp with time zone DEFAULT now())
 RETURNS TABLE(rewarded boolean, eligible boolean, reason text, meaningful_turn_count integer, distinct_meaningful_turn_count integer, duration_seconds integer, event_count integer)
 LANGUAGE plpgsql
...
BEGIN
  ...
  -- claude-review 지적 반영: Gold Key 잔액 상한(50개)은 보상 지급 정책이지 30일
  ...
  -- 전역 활성 Gold Key 상한(50개)은 기존 정책을 그대로 지킨다 — 단, 이벤트 카운트는
