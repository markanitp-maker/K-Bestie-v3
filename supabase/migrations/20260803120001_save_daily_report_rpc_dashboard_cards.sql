...
-- 목적: requests/019-parent-dashboard-15char-summary-fix.md
--       save_and_complete_daily_report_job_v3가 p_report_payload에서
--       dashboard_cards(jsonb, 15자 요약 8개)와 teacher_adults(상세 텍스트)를
--       읽어 daily_reports에 저장하도록 갱신한다. 시그니처는 기존과 동일
--       (p_report_payload jsonb라 이미 임의 키 확장 가능 — 함수 시그니처 변경 없음,
--       DROP 불필요, CREATE OR REPLACE만으로 충분).
-- 원본: supabase/migrations/20260802210000_report_generation_history.sql
