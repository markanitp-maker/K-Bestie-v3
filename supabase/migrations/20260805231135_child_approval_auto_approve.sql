...
ALTER TABLE public.child_approval_requests ADD CONSTRAINT child_approval_requests_status_check 
  CHECK (status IN ('pending', 'PENDING_PAYMENT', 'approved', 'rejected', 'creation_failed', 'CANCELLED', 'PAYMENT_FAILED'));

ALTER TABLE public.child_approval_requests DROP CONSTRAINT IF EXISTS child_approval_requests_approval_method_check;
ALTER TABLE public.child_approval_requests ADD CONSTRAINT child_approval_requests_approval_method_check 
  CHECK (approval_method IN ('BETA_AUTO', 'PAYMENT_AUTO', 'ADMIN_MANUAL') OR approval_method IS NULL);
...
CREATE OR REPLACE FUNCTION public.admin_finalize_child_approval_success(
  ...
  p_admin_email TEXT,
  p_child_id UUID,
  p_approval_method TEXT DEFAULT 'ADMIN_MANUAL'
)
...
REVOKE EXECUTE ON FUNCTION public.admin_finalize_child_approval_success(UUID, UUID, TEXT, UUID, TEXT) FROM PUBLIC, anon, authenticated;
GRANT EXECUTE ON FUNCTION public.admin_finalize_child_approval_success(UUID, UUID, TEXT, UUID, TEXT) TO service_role;
...
BEGIN
  ...
  IF EXISTS (
    ...
    WHERE username = p_username AND status IN ('pending', 'creation_failed', 'PENDING_PAYMENT')
  ) THEN
    RETURN QUERY SELECT false, 'username_taken'::TEXT, NULL::UUID;
    RETURN;
  END IF;
  ...
  INSERT INTO public.child_approval_requests (
    ...
    grade, interests, guardian_consent, guardian_consent_version, status
  ) VALUES (
    ...
    p_grade, p_interests, p_guardian_consent, p_guardian_consent_version, p_status
...
REVOKE EXECUTE ON FUNCTION public.create_child_approval_request(UUID, UUID, UUID, TEXT, TEXT, TEXT, TEXT, TEXT, TEXT, TEXT, TEXT, TEXT, TEXT[], BOOLEAN, TEXT, TEXT) FROM PUBLIC, anon, authenticated;
GRANT EXECUTE ON FUNCTION public.create_child_approval_request(UUID, UUID, UUID, TEXT, TEXT, TEXT, TEXT, TEXT, TEXT, TEXT, TEXT, TEXT, TEXT[], BOOLEAN, TEXT, TEXT) TO service_role;
