-- 관리자 "수집 후 리포트 즉시 생성" Production 실사용에서 Context Correction·Memory
-- Batch·Daily Report가 PERMANENT_ERROR로 실패한 근본원인 조사 중 발견한 별도 결함
-- 수정 — enqueue_collection_jobs_v3()가 특정 호출 조합(단일 아이 + downstream 포함)에서
-- "column reference execution_id is ambiguous"(42702) 런타임 오류를 낼 수 있었다.
--
-- 원인: RETURNS TABLE(execution_id uuid, ...)의 execution_id 출력 컬럼이 함수 전체에서
-- 암묵적 PL/pgSQL 변수로 선언되는데, 함수 본문 내부의
-- INSERT INTO public.pipeline_execution_items (execution_id, ...) ...
-- ON CONFLICT (execution_id, child_id, item_key) DO UPDATE ...
-- 구문에서 pipeline_execution_items.execution_id 테이블 컬럼과 이름이 충돌해
-- Postgres가 어느 쪽을 가리키는지 모호하다고 판단할 수 있었다(실제 재현 확인).
--
-- 수정: 출력 컬럼명을 out_execution_id로 바꿔 충돌을 없앤다. 이 컬럼 값은
-- lib/batch/collection.ts의 호출부에서 전혀 읽지 않으므로(enqueued_count/existing_count/
-- cutoff_at만 사용) 이름 변경이 안전하다.
...
REVOKE EXECUTE ON FUNCTION public.enqueue_collection_jobs_v3(integer, date, uuid, uuid, timestamptz, boolean) FROM PUBLIC, anon, authenticated;
