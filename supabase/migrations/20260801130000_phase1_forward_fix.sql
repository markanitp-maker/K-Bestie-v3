-- Forward Fix: v3 Collection Pipeline RPC Refactoring & Control Table
-- Replaces JS-trusted JSONB merge with pure DB-transactional lock & select.

-- 1. Create Control Table for v3 Activation
CREATE TABLE public.pipeline_v3_control (
...
INSERT INTO public.pipeline_v3_control (id, enabled) VALUES (1, false) ON CONFLICT DO NOTHING;

ALTER TABLE public.pipeline_v3_control ENABLE ROW LEVEL SECURITY;
CREATE POLICY "service_role_only_control" ON public.pipeline_v3_control 
    FOR ALL USING (auth.role() = 'service_role');
GRANT ALL ON public.pipeline_v3_control TO anon, authenticated;

-- 2. Drop previously unsafe RPC
DROP FUNCTION IF EXISTS public.merge_raw_collection_batch(UUID, UUID, DATE, JSONB);

-- 3. Create the strictly isolated, DB-level locking Collection RPC
