...
test("핸즈프리는 TTS 종료 뒤 안전 조건을 모두 만족할 때만 재개한다", () => {
...
test("STT 오류가 한도까지 누적되면 핸즈프리 재개를 멈춘다", () => {
...
test("타이핑 모드에서는 마이크가 자동으로 다시 열리지 않는다", () => {
  // 자동 **재개**는 핸즈프리 전용이다. 타이핑 모드에서는 부모가 누를 때만 열린다.
  assert.equal(shouldResumeHandsFree({ ...readyToResume, mode: "typing" }), false);

  // 자동 **재생**은 다르다 — 기준이 모드가 아니라 질문 경로다.
  // 처음 이 테스트는 `shouldAutoPlayKAnswer("voice", "답변", "typing") === false` 를
  // 단정했는데, 그러면 타이핑 모드에서 마이크로 물었을 때 답을 안 읽어 준다.
...
test("자동 재생 기준은 모드가 아니라 질문 경로다", () => {
  ...
  // 지시서 §9 는 "음성 질문으로 시작한 경우에만" 이지 "핸즈프리일 때만" 이 아니다.
