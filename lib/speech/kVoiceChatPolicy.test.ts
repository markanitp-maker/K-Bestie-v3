...
test("음성 질문의 K 답변만 자동 재생한다", () => {
  ...
  assert.equal(shouldAutoPlayKAnswer("voice", "오늘 서아는 즐거웠대요", "hands-free"), true);
...
test("자동 재생: 답변이 비어 있으면 음성 질문이어도 재생하지 않는다", () => {
  ...
  assert.equal(shouldAutoPlayKAnswer("voice", "", "hands-free"), false);
  assert.equal(shouldAutoPlayKAnswer("voice", "   ", "hands-free"), false);
  assert.equal(shouldAutoPlayKAnswer("voice", "\n\t", "hands-free"), false);
...
test("자동재생 판정은 fallback 문구가 아니라 API 원본으로 한다", () => {
  ...
  for (const raw of [rawEmpty, rawNull, rawBlank]) {
    assert.equal(shouldAutoPlayKAnswer("voice", raw, "hands-free"), false, JSON.stringify(raw));
  }

  // fallback 문구를 그대로 넘기면 통과해 버린다 — 이게 놓쳤던 경로다.
  assert.equal(shouldAutoPlayKAnswer("voice", "응답을 가져올 수 없어요.", "hands-free"), true);

  // 진짜 답변이 있으면 읽는다.
  assert.equal(
    shouldAutoPlayKAnswer("voice", resolveAutoPlaySource("오늘 서아는 즐거웠대요"), "hands-free"),
    true
  );
});
