...
export function formatRelationshipMemory(snapshot: RelationshipMemorySnapshot): string {
  ...
  lines.push(
    ...
    // 2026-08-17 Dev QA 실측: 아이가 "지난주에 놀이공원 갔다고 했잖아"처럼 **단정해서**
    // 말하자 케이가 "아 맞다, 놀이공원 갔다고 했었지!"라고 없는 기억에 맞장구쳤다.
    // 의문형("했었지?")에는 방어했는데 단정형에는 그대로 동의했다.
    // 기억 못 하는 건 아쉬운 정도지만, 안 한 얘기를 맞다고 하는 건 아이를 속이는 것이다.
    "- 아이가 '내가 ~라고 했잖아'처럼 단정해서 말해도, 위 기억에 없는 내용이면 맞다고 하지 마.",
    "- 그럴 때는 '그건 기억이 잘 안 나네, 다시 말해줄래?'처럼 솔직하게 말하고 아이에게 물어봐.",
    "- 위 기억에 없는 사건·장소·사람 이름을 네가 먼저 말하거나 확인해 주는 것은 절대 금지야.",
