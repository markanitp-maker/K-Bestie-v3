...
const CHOSUNG_HINT_KWS = [
  ...
  "몰라", "몰르", "모르겠어", "잘 모르",
  // 016 후속 대표님 실사용(2026-08-20 10:46) — 아이가 "흰트줘" 라고 쳤는데 어떤 신호에도
  // 안 걸려 초성게임이 그 턴을 처리하지 못했다. 그러면 자유대화로 빠진 케이가 대화
  // 맥락에 있던 **정답을 그대로 말해 버린다**(초성 ㅋㄲㄹ → "코끼리야!").
  // 아이는 "힌트 달라고 했더니 정답을 알려주네?" 라고 했다.
  // 정답 유출 가드는 answerMustNotAppear 가 설정된 턴에만 도는데, 스킬이 턴을 처리하지
  // 못하면 그 값이 없어 가드가 아예 안 돈다. 그래서 **인식 폭을 넓히는 것이 가드다.**
  //
  // 아이는 키보드로 치고 음성인식도 거친다. 흔한 변형을 함께 받는다.
  "흰트", "핀트", "힌투", "힌드", "힌튜", "힌트줘", "hint",
  "모루", "모룰", "몰라겠", "머르겠", "머겠",
];
