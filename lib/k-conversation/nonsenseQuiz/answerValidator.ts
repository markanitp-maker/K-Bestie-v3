 ...
 *
 * 2026-08-20 20:07 실측 보강 — `어`, `네`, `예` 를 추가했다. 케이가 "하나 더 낼까?"
 * 라고 물었을 때 아이가 이렇게 답하는데 오답으로 채점됐다.
 * DB 실측으로 안전을 확인했다: 한 글자 정답이 39개 있으나(해·책·빗·못·꿈·글·물·달·꽃 등)
 * `어`·`네`·`예`·`응`·`음` 을 정답으로 두는 문항은 **0개**다.
 * `해` 처럼 실제 정답인 토큰은 여전히 넣지 않는다.
 */
const BARE_AFFIRMATIVE_PATTERN =
  /^(?:ㅇㅇ|ㅇㅋ|ㄱㄱ|응+|웅+|넹|어+|네|예|그래|그럼|좋아|조아|좋지|좋아요|당연|당연하지|콜|오케이|ok|okay)[!?.~^ㅋㅎ\s]*$/i;
...
/** 정답 바로 공개 / 포기 요청 패턴 */
//
// 2026-08-20 20:07 대표님 QA 실측 — `종답은?` (정답은? 오타)이 화제 전환으로 분류돼
// 넌센스 세션이 닫혔다(20:07:42). 그래서 그다음 `ㅇㅇ` 이 왔을 때는 이미 놀이가 없었고
// 케이가 자유대화로 "다음 퀴즈는 어떤 걸로 내볼까?" 라고 되물었다.
// 대표님: "ㅇㅇ 이라고 했는데, 추가 문제 안 내고, 종료됨"
//
// 직전 수정에서 `정답이 뭐야?` 만 받게 했는데, 아이는 `정답은?` 처럼 **조사+물음표**로도
// 묻는다. 그리고 아이는 오타를 낸다(종답/정단/정담).
const REVEAL_ANSWER_PATTERNS = [
  /(?:정답|답|종답|정단|정담)(?:이|가)?\s*(?:알려줘|알려\s*줘|공개|뭐야|뭔데|말해줘|말해\s*줘|보여줘|알려줄래)/,
  /(?:모르겠어|몰라)\s*(?:정답|답)\s*(?:알려줘|뭐야|뭔데)/,
  // `정답은?`, `답은?`, `종답은?` — 조사만 붙이고 물음표로 끝내는 형태.
  /^(?:정답|답|종답|정단|정담)(?:은|는|이|가)?\s*[?？]+\s*$/,
  /^(?:정답|답|종답|정단|정담)$/,
