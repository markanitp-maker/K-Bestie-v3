 ...
 *  (record_mission_event_completion, migration 20260810170000)가 KST 날짜별로
 *  원자적·멱등 처리하므로 여기서는 mission_complete 활동 파라미터만 넘긴다.
 *  실패해도 미션 완료 응답 자체는 막지 않는다
...
export async function recordMissionOnboardingCompletion(
  ...
  try {
    const { error } = await service.rpc("record_mission_event_completion", {
      ...
      p_activity_type: "mission_complete",
      p_source_session_id: missionSessionId,
      p_activity_completed_at: completedAt.toISOString(),
    });
    if (error) {
