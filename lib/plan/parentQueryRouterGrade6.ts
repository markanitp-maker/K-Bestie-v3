// requests/request-parent-query-router-grade6-v1.md — 초등학교 6학년 정책 데이터.
// §13: "4학년 안정화 이후 별도 검증하고, 외모·이성·SNS 영역은 상담·법률 검토 완료 후
...
export const POLICY_VERSION = "PQR-G6-1.0";
export const APPLICABLE_GRADE = 6;
...
export const GREEN_RULES: readonly GreenRule[] = [
  ...
  { id: "G-04", area: "food_pref", parentDraftText: "요즘 꽂힌 음식이 있는지 물어볼까요?", childQuestionText: "요즘 꽂힌 음식 있어?" },
  { id: "G-05", area: "pride", parentDraftText: "최근 뿌듯하게 느낀 일이 있는지 물어볼까요?", childQuestionText: "최근에 '이건 좀 뿌듯했다' 싶은 거 있었어?" },
  { id: "G-06", area: "content", parentDraftText: "요즘 가장 자주 보는 콘텐츠가 무엇인지 물어볼까요?", childQuestionText: "요즘 제일 자주 보는 거 뭐야?" },
...
// §7 RED LIST — R-06~R-08(외모·이성·SNS)은 강한 Red 게이트, 단순 키워드가 아니라
// 캐묻거나 감시하려는 의도 확인 시 확정(grade5와 동일 원칙).
export const RED_RULES: readonly RedRule[] = [
  {
    id: "R-01",
    area: "emotion_cause",
    pattern: /(때문인\s*것\s*같|왜\s*그런지|무슨\s*일\s*있었는지\s*캐|기분\s*안\s*좋.{0,10}(이유|원인))/,
    coachingText: "6학년은 원인을 캐물으면 방어부터 할 수 있어요. 케이는 대신 캐묻지 않고, 직접은 '무슨 일 있으면 언제든 이야기해'처럼 여지만 열어 주세요.",
  ...
  },
  {
    ...
    coachingText:
      "이 나이에는 친구 문제를 캐물으면 오히려 입을 닫을 수 있어요. 판단 없이 들을 준비를 보여 주고, 소외·괴롭힘이 의심되면 담임·상담 선생님과 상의해 주세요.",
    safeAlternativeGreenId: "G-02",
  },
  {
    id: "R-03",
    ...
    coachingText: "성적 압박은 대화를 끊는 원인이 될 수 있어요. 결과보다 노력과 아이가 느끼는 부담에 반응해 주세요.",
...
const CONFIG: RouterPolicyConfig = {
  ...
  greenAreaPromptGuide:
    "interest(관심사), school_fun(학교에서 기억에 남은 순간), subject_like(관심 가는 과목), food_pref(꽂힌 음식), pride(뿌듯했던 일), content(콘텐츠·유행), weekend(주말 계획), dream(관심 있는 진로)",
  redAreaPromptGuide:
...
export async function routeParentQueryGrade6(
  ai: GenAILikeClient,
  model: string,
  rawText: string,
