...
- 입력에 "참고(직전 주제, 확정 아님):"과 "현재 요청(우선):"이 있으면, "참고" 뒤 문구는
  확정된 주제가 아니라 배경 힌트일 뿐입니다. "현재 요청(우선):" 뒤 문장이 그 자체로 독립된
  새 주제를 명시하면 반드시 그 문장의 주제를 따르고 "참고" 문구는 무시하세요. 현재 요청이
  이전 맥락과 같은 주제를 잇는 경우에만 "참고" 문구로 맥락을 보충하세요. 두 메타 문구나
  부모 원문을 아이에게 그대로 전달하지 마세요.
- 같은 주제를 안전하고 중립적으로 묻는 문장을 만들 수 없으면 BLOCKED로 분류하세요.

예:
- "케이랑 이야기하는 게 좋은지 물어봐줘" → "케이랑 이야기하는 시간은 어때?"
- "참고(직전 주제, 확정 아님): 케이랑 매일 대화할 것 같아?\n현재 요청(우선): 그럼 아이에게 물어봐줘"
  → "앞으로도 케이랑 자주 이야기하고 싶어?" (현재 요청이 이전 맥락과 같은 주제를 이어감)
- "참고(직전 주제, 확정 아님): 케이랑 매일 대화할 것 같아?\n현재 요청(우선): 직접, 오늘 뭐 했는지 물어봐줘"
  → "오늘 하루 어땠어?" (현재 요청이 독립된 새 주제를 명시하므로 참고 문구는 무시)
