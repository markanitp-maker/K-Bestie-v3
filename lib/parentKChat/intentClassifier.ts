...
// 부모의 육아 상담·조언 요청. **아이에 대한 사실 조회가 아니다.**
//
// 2026-08-18 Dev 실측 사고: "우리 애가 요즘 통 말이 없어요" / "왜 그럴까요?" /
// "그럼 제가 어떻게 하면 좋을까요?" 가 전부 CHILD_INFORMATION_QUERY 로 분류돼
// RAG 를 뒤졌고, 근거가 없으니 "기록이 없어요" 만 세 번 반복했다.
// 부모는 상담하러 왔는데 검색 결과 없음만 돌려받았다.
//
// 가르는 기준: 특정 시점의 **행동·사건을 묻는 것**은 조회다.
// 아이의 **상태를 서술하거나 원인·대처를 묻는 것**은 상담이다.
// 특정 시점의 행동·사건을 묻는 조회 표현. 상담 패턴과 겹치면 **조회가 이긴다.**
// 아이에 대해 지어내는 것이 부모가 답답한 것보다 훨씬 나쁘다.
const CHILD_FACT_LOOKUP_PATTERNS = [
  /(?:어제|오늘|그제|엊그제|지난주|이번\s*주|저번\s*주|주말|그날)(?:에|는)?\s*(?:뭐|무슨|어떤|무엇)/,
  /(?:뭐|무엇)\s*(?:했|하고\s*있었|먹었|말했|얘기했)/,
  /(?:무슨|어떤)\s*(?:얘기|이야기|말|활동|일)(?:을|를)?\s*(?:했|하고)/,
  /기록(?:을|이|은)?\s*(?:보여|알려|확인)/,
  /리포트(?:를|에)?\s*(?:보여|알려|확인)/,
  // 아이 상태를 **묻는** 의문형은 조회다 — "요즘 힘들어해?" 는 사실 확인 요청이고,
  // "요즘 힘들어해요" 는 부모가 털어놓는 상담이다. 서술과 의문을 종결로 가른다.
  // ("왜 그럴까요?"·"어떻게 하면 좋을까요?" 는 원인·대처 질문이라 여기 걸리지 않는다.)
  /(?:해|하니|하나|한가|있어|있나|어때|어땠|괜찮)[?？]\s*$/,
];

const PARENTING_ADVICE_PATTERNS = [
  // 상태 서술 — 아이가 어떻더라
  /말(?:이|을)?\s*(?:없|안\s*해|안해)/,
  /(?:힘들어|우울해|불안해|예민해|짜증(?:을)?\s*(?:내|부리)|화(?:를)?\s*내)/,
  /(?:자꾸|계속|맨날|하루\s*종일)\s*(?:울|떼|짜증|싸우)/,
  /(?:밥|잠)(?:을|도)?\s*(?:안\s*먹|못\s*자|안\s*자)/,
  /(?:학교|유치원|어린이집)(?:에)?\s*(?:가기\s*싫|안\s*가려)/,
  /(?:친구|사이)(?:가|와|랑)?\s*(?:문제|안\s*좋|싸)/,
  // 원인 질문 — 왜 그럴까
  /왜\s*(?:그럴|그런|이럴|저럴|그러|이러)/,
  /(?:무슨|어떤)\s*(?:일|문제)(?:일까|이\s*있|가\s*있)/,
  /이유(?:가|는)?\s*(?:뭐|무엇)/,
  // 조언 요청 — 어떻게 하면 좋을까
  /어떻게\s*(?:하면|해야|하죠|하나요|할까)/,
  /어떡(?:하죠|하나요|해요|하지)/,
  /(?:뭘|무엇을|뭐를)\s*(?:해\s*줘야|해야|하면)/,
  /조언(?:\s*(?:좀|해|부탁))/,
  /도와\s*(?:주세요|줘)/,
  // 걱정 표현
  /걱정(?:이야|돼|되|입니다|이에요)/,
...
export function classifyParentKChatIntent(
  ...
  // 육아 상담·조언은 기록 조회가 아니다. 근거가 없다고 돌려보내면 안 된다.
  // 단, 특정 시점의 행동을 묻는 조회 표현이 함께 있으면 조회를 우선한다 —
  // 상담을 조회로 잘못 보내면 "기록이 없어요"로 그치지만(불편),
  // 조회를 상담으로 잘못 보내면 케이가 아이에 대해 지어낸다(사고).
  if (
    PARENTING_ADVICE_PATTERNS.some((p) => p.test(text)) &&
    !CHILD_FACT_LOOKUP_PATTERNS.some((p) => p.test(text))
  ) {
