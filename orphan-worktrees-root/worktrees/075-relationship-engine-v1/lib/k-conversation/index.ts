...
import { buildRelationshipSessionContext } from "@/lib/relationship/relationshipContext";
...
export async function respond(
  ...
  // 3) 4-tier Memory + Core/Grade Persona 병렬 조회. Relationship Session Context는
  // 같은 Memory/Core Promise의 결과를 재사용한다. 따라서 Memory V3를 다시 조회하지
  // 않으면서도 첫 턴 memory_fact_ids와 정규화 학년을 정확히 고정할 수 있다.
  // AGENTS.md 병렬 호출 하드룰: Promise.all 금지, Promise.allSettled 필수(codex-rv 2차 지적 —
  // 최상위 조립부에도 남아 있었다).
  const memoryPromise = loadRelationshipMemory(deps.db, {
      childId: input.childId,
      sessionId: input.sessionId,
      currentUtterance: input.currentUtterance,
  });
  const coreCtxPromise = loadCorePersonaContext(deps.db, input.childId);
  const relationshipSessionContextPromise = Promise.allSettled([
    memoryPromise,
    coreCtxPromise,
  ]).then(([memoryResult, coreCtxResult]) => buildRelationshipSessionContext(
    deps.db,
    {
      childId: input.childId,
      sessionId: input.sessionId,
      grade: coreCtxResult.status === "fulfilled"
        ? coreCtxResult.value.peerPersona.realGrade ?? coreCtxResult.value.peerPersona.gradeLabel
        : null,
      memorySnapshot: memoryResult.status === "fulfilled" ? memoryResult.value : undefined,
    },
  ));
  const [memorySettled, coreCtxSettled, relationshipCtxSettled] = await Promise.allSettled([
    memoryPromise,
    coreCtxPromise,
    relationshipSessionContextPromise,
  ]);
  ...
  if (relationshipCtxSettled.status === "rejected") {
    console.error("[k-conversation/index] buildRelationshipSessionContext rejected", relationshipCtxSettled.reason);
  }
  ...
  const relationshipScenarioFragment = relationshipCtxSettled.status === "fulfilled"
    ? relationshipCtxSettled.value.fragment
    : "";
