...
export interface ResponseGeneratorInput {
  ...
  /** 세션 시작 시 고정된 Relationship Stage/Scenario/Grade Strategy 지침.
   * 없으면 기존 Core/Grade Persona + Memory 조합을 그대로 유지한다. */
  relationshipScenarioFragment?: string;
...
function buildSystemInstruction(input: ResponseGeneratorInput): string {
  ...
  const lines = [
    ...
    input.relationshipScenarioFragment ?? "",
