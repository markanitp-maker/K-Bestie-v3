...
export async function respond(
  ...
  // currentUtterance는 보통 아직 저장 전이라 별도로 append한다. 호출자가 이미
  // finalized로 저장해뒀다고 명시하면(예: Mission v3) sameSession 조회 결과에
  // 이미 포함돼 있으므로 중복 집계를 피한다.
  const boredom = assessBoredom(
    input.currentUtteranceAlreadyInSession
      ? recentChildUtterances
      : [...recentChildUtterances, input.currentUtterance]
  );
