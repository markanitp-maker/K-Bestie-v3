...
import { assessBoredom } from "@/lib/k-conversation/boredomDetection";
...
export async function POST(req: NextRequest) {
  ...
  if (!started.k_response_draft && started.turn_status !== "FINALIZED") {
    ...
    if (safetyOutput) {
      ...
      // EngineOutput intentionally exposes only the chosen action/category, not
      // its internal BoredomAssessment. Use the shared detector against the
      // exact same finalized same-session turns the engine reads
      // (fetchSameSessionTurns — finalized only, last 6). v3's start RPC
      // already finalizes the child's message before this call, so the
      // current turn's utterance is already the newest entry here — do not
      // append answerText separately or it gets counted twice.
      const [sameSessionSettled, recentHistorySettled] = await Promise.allSettled([
        fetchSameSessionTurns(service, session.child_id, sessionId),
        fetchRecentMissionHistory({ db: service, sessionId, currentTurnId: clientTurnId }),
      ]);

      if (sameSessionSettled.status === "fulfilled") {
        boredomEarlyFinish = assessBoredom(
          sameSessionSettled.value.filter((turn) => turn.role === "child").map((turn) => turn.content)
        ).suggestedAdjustment?.allowEarlyFinish === true;
      } else {
        // Fail-open, matching the engine's own Promise.allSettled memory lookups:
        // a transient same-session lookup failure should not block the turn.
        console.error("[mission/v3/turn] same-session boredom lookup failed", sameSessionSettled.reason);
        boredomEarlyFinish = false;
      }
      const recentHistory = recentHistorySettled.status === "fulfilled" ? recentHistorySettled.value : [];
      if (recentHistorySettled.status === "rejected") {
        console.error("[mission/v3/turn] recent mission history lookup failed", recentHistorySettled.reason);
      }
